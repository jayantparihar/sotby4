...
const getUsers = (year) => {
  return new Promise(function (resolve, reject) {
    pool.query(`SELECT u.username, u.first_name, u.last_name, u.row_num, c.course_num, ca.ca_id, ca.start_date, ca.end_date, ca.m_or_a, c.title, c.colour, v.start_date as vacation_start, v.end_date as vacation_end, v.vacation_id, v.approved, r.ra_id, r.model_name, r.model_num, r.start_date as resource_start, r.end_date as resource_end, r.quantity from "user" u
                  LEFT JOIN course_assignment ca ON u.username = ca.username
                  LEFT JOIN resource_allocation r ON r.username = u.username
...
const postCourseAssignment = (course) => {
  return new Promise(function (resolve, reject) {
    pool.query(`
            ...
            (username, course_num, start_date, end_date, m_or_a)
            VALUES
            ('${course.instructorKey}', ${course.number}, 
            to_timestamp(${course.start} / 1000.0), to_timestamp(${course.end} / 1000.0), ${course.m_or_a})
            RETURNING ca_id;
            `
      , (error, results) => {
        if (error) {
...
const putCourse = (username, id, start, end, m_or_a) => {
  return new Promise(function (resolve, reject) {
    pool.query(`UPDATE "course_assignment" 
    SET start_date = (to_timestamp(${start} / 1000.0)), end_date = (to_timestamp(${end} / 1000.0)), m_or_a = '${m_or_a}' 
    WHERE ca_id = ${id}`,
      (error, results) => {
        if (error) {
          reject(error)
...
const getCourseDetail = (course_num) => {
  return new Promise(function (resolve, reject) {
    pool.query(`SELECT daily_schedule.ds_id, course_assignment.ca_id, date, description, course_assignment.username, subject, course 
                FROM daily_schedule
                ...
                GROUP BY (daily_schedule.ds_id, course_assignment.ca_id, date, description, course_assignment.username, subject, course)
                ORDER BY date;`,
      (error, results) => {
        if (error) {
          reject(error)
...
const getResources = (date) => {
  console.log(date);
  return new Promise(function (resolve, reject) {
    pool.query(`SELECT resource.model_num, resource.model_name, quantity_total, model_location, (quantity_total - quantity) AS q_left
                FROM "resource"
                LEFT JOIN
...
const bookChanged = (bookingInfo) => {
  return new Promise(function (resolve, reject) {
    pool.query(`UPDATE "resource_allocation"
                SET start_date = (to_timestamp(${bookingInfo.start} / 1000.0)), end_date = (to_timestamp(${bookingInfo.end} / 1000.0))
                WHERE ra_id=${bookingInfo.raId};`,
      (error, results) => {
        if (error) {
          reject(error)
        }
...
const bookResource = (bookingInfo) => {
  return new Promise(function (resolve, reject) {
    pool.query(`INSERT INTO "resource_allocation"
                (username, model_num, model_name, start_date, end_date, quantity)
                VALUES 
                ('${bookingInfo.username}', ${bookingInfo.model_num}, '${bookingInfo.model_name}', '${bookingInfo.date}', '${bookingInfo.date}',  ${bookingInfo.quantity_booked})`,
...
const deleteBookedResource = (raId) => {
  return new Promise(function (resolve, reject) {
    pool.query(`
